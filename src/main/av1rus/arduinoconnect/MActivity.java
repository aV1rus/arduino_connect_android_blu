...
import av1rus.arduinoconnect.MainFragments.AutoDataFragment;
import av1rus.arduinoconnect.MainFragments.BrightnessFragment2;
import av1rus.arduinoconnect.MainFragments.MyFragment;
import av1rus.arduinoconnect.MainFragments.PresetsFragment;
...
public class MActivity extends Main {
	...
	// Tabs
	private static final String[] CONTENT = new String[] { "Presets", "Brightness", "Vehicle", "OBD Data", "Data Log" };
...
*/
	...
	public void updateMain(Intent intent) { // Overriding from Mains Broadcast
		...
		if (tag == BluetoothService.HEAD_HALO_CHANGE) {
			shouldBeSend = false;
			updateHeadHalos();
			shouldBeSend = true;
		} else if (tag == BluetoothService.FOG_HALO_CHANGE) {
			shouldBeSend = false;
			updateFogHalos();
			shouldBeSend = true;
		} else if (tag == BluetoothService.INTERIOR_CHANGE) {
			shouldBeSend = false;
			updateInterior();
			shouldBeSend = true;
		} else if (tag == BluetoothService.FOG_CHANGE) {
			shouldBeSend = false;
			updateFogs();
			shouldBeSend = true;
		}
